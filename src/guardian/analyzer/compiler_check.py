...
_RANGED_VULNERABILITIES: list[
    tuple[str, tuple[int, int, int], tuple[int, int, int], Severity, str]
] = [
    (
        "Vyper 0.2.15-0.3.0 - reentrancy vulnerability in generated bytecode paths "
        "(historically exploited in Curve ecosystem incidents).",
        (0, 2, 15),
        (0, 3, 0),
        Severity.CRITICAL,
        "GHSA-4g63-c64m-25w9",
    ...
    (
        "Vyper 0.3.1-0.3.7 - storage-layout corruption risk in specific compilation paths.",
        (0, 3, 1),
        (0, 3, 7),
        Severity.HIGH,
        "GHSA-6m97-7527-mh74",
    ...
    (
        "Vyper 0.3.0-0.3.9 - bounds-checking bug affecting selected arithmetic/array code paths.",
        (0, 3, 0),
        (0, 3, 9),
        Severity.HIGH,
        "GHSA-ph9x-4vc9-m39g",
...
def check_compiler_version(contract: ContractInfo) -> list[DetectorResult]:
    ...
    for desc, vmin, vmax, severity, advisory in _RANGED_VULNERABILITIES:
        ...
        results.append(
            DetectorResult(
                ...
                why_not_suppressed="Advisory range match.",
            )
        )
